...
sed -e "s#file:///home/worker/workspace/build/tests/reftest/tests/##g" |
...
while read op f1 f2; do
    ...
    ls reftest*.list | grep -v stylo | xargs sed -i -e "s@\($op ${f1##*/} * ${f2##$folder/}\)@fails-if(webrender) \1@"
    ls reftest*.list | grep -v stylo | xargs sed -i -e "s@\($op ${f1##*/} * ${f2##$subfolder/}\)@fails-if(webrender) \1@"
    # deduplicate
    ls reftest*.list | grep -v stylo | xargs sed -i -e "s@fails-if(webrender) fails-if(webrender)@fails-if(webrender)@"
    ls reftest*.list | grep -v stylo | xargs sed -i -e "s@fails-if(webrender) fails-if(webrender)@fails-if(webrender)@"
    # if there is an HTTP on the line, move the fails-if to before that
    ls reftest*.list | grep -v stylo | xargs sed -i -e "s@\(HTTP.*\) fails-if(webrender)@fails-if(webrender) \1@"

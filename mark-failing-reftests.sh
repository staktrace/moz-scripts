...
sed -e "s#http://10.0.2.2:8854/tests/##g" |
...
while read op f1 f2; do
    ...
    ls reftest*.list | xargs grep "$op ${f1##*/} * ${f2##$folder/}" > /dev/null
    if [ $? -eq 0 ]; then
        popd >/dev/null
        continue
    fi
    ls reftest*.list | xargs grep "$op ${f1##*/} * ${f2##$subfolder/}" > /dev/null
    if [ $? -eq 0 ]; then
        popd >/dev/null
        continue
    fi
    if [[ $folder == layout/reftests/w3c-css/received/* ]]; then
        ...
        ls reftest.list | xargs grep "$op ${f1#$folder} * ${f2#$folder}" > /dev/null
        if [ $? -eq 0 ]; then
            popd >/dev/null
            continue
        fi
...
while read op f1 f2; do
    ...
    ls reftest*.list | xargs sed -i -e "s@\($op ${f1##*/} * ${f2##$folder/}\)@fails-if(geckoview) \1@"
    ls reftest*.list | xargs sed -i -e "s@\($op ${f1##*/} * ${f2##$subfolder/}\)@fails-if(geckoview) \1@"
    if [[ $folder == layout/reftests/w3c-css/received/* ]]; then
        folder="layout/reftests/w3c-css/received/"
        cd $MOZILLA_SRC/$folder
        ls reftest.list | xargs sed -i -e "s@\($op ${f1#$folder} * ${f2#$folder}\)@fails-if(geckoview) \1@"
    fi
    # deduplicate
    ...
    ls reftest*.list | xargs sed -i -e "s@\(HTTP.*\) fails-if(geckoview)@fails-if(geckoview) \1@"
